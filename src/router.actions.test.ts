...
import type { ParamsT, ParsedRouteT, RoutesT, RouteT, UserT } from '.'
...
describe('Router Actions', () => {
  ...
  describe('mock verification', () => {
    test('should verify mocks are working', () => {
      const mockRouterEffects = overmind.effects
  ...
  describe('initializeRouter', () => {
    test('should initialize router with current route', () => {
      ...
      overmind.actions.initializeRouter(routes)

      expect(overmind.state.current).toBe('ROUTER_READY')
      expect(
        overmind.state.current === 'ROUTER_READY' && overmind.state.currentRoute
      ).toEqual({
        pattern: '/users/:id',
        path: '/users/123',
        routeParams: { id: '123' },
    ...
    test('should handle route not found during initialization', () => {
      ...
      expect(overmind.state.current).toBe('ROUTE_NOT_FOUND')
      expect(
        overmind.state.current === 'ROUTE_NOT_FOUND' &&
          overmind.state.requestedPath
      ).toBe('/invalid/path')
    })
  })
  ...
  describe('getCurrentRoute', () => {
    test('should return current route', () => {
      overmind.actions.initializeRouter(routes)
      const currentRoute = overmind.actions.getCurrentRoute()
  ...
  describe('navigateTo', () => {
    ...
    test('should navigate to simple route', () => {
      ...
      expect(overmind.state.current).toBe('ROUTER_READY')
      expect(
        overmind.state.current === 'ROUTER_READY' &&
          overmind.state.currentRoute.pattern
      ).toBe('/')
    })
    ...
    test('should navigate to route with parameters', () => {
      ...
      expect(
        overmind.state.current === 'ROUTER_READY' &&
          overmind.state.currentRoute.routeParams
      ).toEqual({
        id: '123',
      })
    })
    ...
    test('should handle navigation error for invalid route', () => {
      overmind.actions.navigateTo('/invalid/route')

      expect(overmind.state.current).toBe('NAVIGATION_FAILURE')
      if (overmind.state.current === 'NAVIGATION_FAILURE') {
        expect(overmind.state.errorMsg).toContain('Invalid route pattern')
        expect(overmind.state.errorType).toBe('invalid_pattern')
      }
    })
  })
  ...
  describe('navigateForward', () => {
    ...
    test('should call history.forward()', () => {
      overmind.actions.navigateForward()
  ...
  describe('browser navigation handling', () => {
    ...
    test('should handle popstate event', () => {
      ...
      overmind.actions.onPopState()

      expect(overmind.state.current).toBe('ROUTER_READY')
      if (overmind.state.current === 'ROUTER_READY') {
        expect(overmind.state.currentRoute.routeParams!.id).toBe('456')
        expect(overmind.state.currentRoute.params?.tab).toBe('settings')
      }
    })
    ...
    test('should handle popstate to route not found', () => {
      ...
      if (overmind.state.current === 'ROUTE_NOT_FOUND') {
        expect(overmind.state.requestedPath).toBe('/invalid/path')
      }
    })
  })
  ...
  describe('complex navigation scenarios', () => {
    ...
    test('should handle multiple rapid navigations', () => {
      ...
      expect(overmind.state.current).toBe('ROUTER_READY')
      if (overmind.state.current === 'ROUTER_READY') {
        expect(overmind.state.currentRoute.pattern).toBe('/users/:id')
        expect(overmind.state.currentRoute.routeParams!.id).toBe('123')
      }
      ...
      expect(overmind.state.current).toBe('ROUTER_READY')
      if (overmind.state.current === 'ROUTER_READY') {
        expect(overmind.state.currentRoute.pattern).toBe('/clients/:id/cars')
        expect(overmind.state.currentRoute.routeParams!.id).toBe('789')
      }
    })
    ...
    test('should preserve state through navigation failure and recovery', () => {
      ...
      // Recovery navigation
      overmind.actions.navigateTo('/login')
      expect(overmind.state.current).toBe('ROUTER_READY')
      if (overmind.state.current === 'ROUTER_READY') {
        expect(overmind.state.currentRoute.pattern).toBe('/login')
      }
    })
    ...
    test('should handle navigation with complex route parameters', () => {
      ...
      if (overmind.state.current === 'ROUTER_READY') {
        expect(overmind.state.currentRoute.routeParams).toEqual({
          id: '123',
          carId: '456',
        })
        expect(overmind.state.currentRoute.params).toEqual({
          action: 'edit',
          modal: 'true',
        })
      }
  ...
  describe('error handling', () => {
    ...
    test('should handle navigation errors gracefully', () => {
      ...
      overmind.actions.navigateTo('/totally/invalid/pattern')
      expect(overmind.state.current).toBe('NAVIGATION_FAILURE')
    })
  ...
  describe('redirectTo', () => {
    ...
    test('should handle redirect error for invalid route', () => {
      overmind.actions.redirectTo('/invalid/route')
    ...
    test('should redirect with URL string', () => {
      overmind.actions.redirectTo('/users/456?tab=settings')
  ...
  describe('updateParams', () => {
    ...
    test('should handle updateParams when not in ROUTER_READY state', () => {
      const notReadyOvermind = createOvermindMock(
        ...
        (state) => {
          state.current = 'ROUTE_NOT_FOUND'
          if (state.current === 'ROUTE_NOT_FOUND') {
            state.requestedPath = '/invalid'
            state.currentRoute = undefined
          }
        }
      )

      notReadyOvermind.actions.updateParams({
        params: { test: 'value' },
      })

      expect(notReadyOvermind.state.current).toBe('ROUTE_NOT_FOUND')
    })
  })
  ...
  describe('URL string navigation', () => {
    ...
    test('should navigate using URL string with query parameters', () => {
      overmind.actions.navigateTo('/users/123?tab=profile')

      expect(overmind.state.current).toBe('ROUTER_READY')
      if (overmind.state.current === 'ROUTER_READY') {
        expect(overmind.state.currentRoute.pattern).toBe('/users/:id')
        expect(overmind.state.currentRoute.routeParams!.id).toBe('123')
        expect(overmind.state.currentRoute.params?.tab).toBe('profile')
      }
    })
    ...
    test('should navigate using URL string without query parameters', () => {
      ...
      if (overmind.state.current === 'ROUTER_READY') {
        expect(overmind.state.currentRoute.pattern).toBe('/login')
        expect(overmind.state.currentRoute.params).toEqual({}) // Route has expected query params, so include empty object
        expect(overmind.state.currentRoute.routeParams).toBeUndefined() // No route params in URL path
      }
    })
    ...
    test('should navigate using complex URL string', () => {
      ...
      expect(overmind.state.current).toBe('ROUTER_READY')
      if (overmind.state.current === 'ROUTER_READY') {
        expect(overmind.state.currentRoute.pattern).toBe(
    ...
    test('should handle malformed URL string', () => {
      overmind.actions.navigateTo('not-a-valid-url?but&has=params')
  ...
  describe('advanced error handling', () => {
    ...
    test('should handle browser navigation errors in navigateBack', () => {
      ...
      try {
        ...
        expect(overmind.state.current).toBe('NAVIGATION_FAILURE')
        if (overmind.state.current === 'NAVIGATION_FAILURE') {
          expect(overmind.state.errorMsg).toContain('Browser navigation failed')
          expect(overmind.state.errorType).toBe('browser_navigation_error')
        }
      } finally {
    ...
    test('should handle browser navigation errors in navigateForward', () => {
      ...
      try {
        ...
        expect(overmind.state.current).toBe('NAVIGATION_FAILURE')
        if (overmind.state.current === 'NAVIGATION_FAILURE') {
          expect(overmind.state.errorMsg).toContain('Forward navigation failed')
          expect(overmind.state.errorType).toBe('browser_navigation_error')
        }
      } finally {
    ...
    test('should handle navigation error in navigateTo', () => {
      const errorOvermind = createOvermindMock(config, {
        ...
        validateRoute: (pattern: string, routes: RoutesT) => pattern in routes,
        parseRoute: (routeTo: RouteT) =>
          typeof routeTo === 'string'
            ? { pattern: routeTo, path: routeTo }
            : { ...routeTo, path: '/mock-path' },
      ...
      errorOvermind.actions.initializeRouter(routes)

      errorOvermind.actions.navigateTo({
        pattern: '/users/:id',
        routeParams: { id: '123' },
      })

      expect(errorOvermind.state.current).toBe('NAVIGATION_FAILURE')
      if (errorOvermind.state.current === 'NAVIGATION_FAILURE') {
        expect(errorOvermind.state.errorMsg).toContain('Navigation failed')
        expect(errorOvermind.state.errorType).toBe('navigation_error')
      }
    })
    ...
    test('should handle redirect error in redirectTo', () => {
      ...
      errorOvermind.actions.redirectTo({
        pattern: '/users/:id',
        routeParams: { id: '123' },
      })
      ...
      expect(errorOvermind.state.current).toBe('NAVIGATION_FAILURE')
      if (errorOvermind.state.current === 'NAVIGATION_FAILURE') {
        expect(errorOvermind.state.errorMsg).toContain('Redirect failed')
        expect(errorOvermind.state.errorType).toBe('redirect_error')
      }
    })
    ...
    test('should handle getCurrentRoute returning null during navigation', () => {
      ...
      expect(errorOvermind.state.current).toBe('ROUTE_NOT_FOUND')
      if (errorOvermind.state.current === 'ROUTE_NOT_FOUND') {
        expect(errorOvermind.state.requestedPath).toBeDefined()
      }
    })
    ...
    test('should handle getCurrentRoute returning null during browser navigation', () => {
      ...
      errorOvermind.actions.navigateBack()

      expect(errorOvermind.state.current).toBe('ROUTE_NOT_FOUND')
      if (errorOvermind.state.current === 'ROUTE_NOT_FOUND') {
        expect(errorOvermind.state.requestedPath).toBe(window.location.pathname)
      }
    })
  })
  ...
  describe('state machine transitions from different states', () => {
    test('should handle navigation from NAVIGATION_FAILURE state', () => {
      ...
      overmind.actions.navigateTo('/invalid/pattern')
      expect(overmind.state.current).toBe('NAVIGATION_FAILURE')
    ...
    test('should handle router initialization from NAVIGATION_FAILURE state', () => {
      const failureOvermind = createOvermindMock(
        ...
        (state) => {
          state.current = 'NAVIGATION_FAILURE'
          if (state.current === 'NAVIGATION_FAILURE') {
            state.errorMsg = 'Test error'
            state.errorType = 'test'
            state.currentRoute = undefined
          }
        }
      )
      ...
      mockLocation.pathname = '/'
      mockLocation.href = 'http://localhost:3000/'
      failureOvermind.actions.initializeRouter(routes)
      expect(failureOvermind.state.current).toBe('ROUTER_READY')
    })
  })
  ...
  describe('Route Guards', () => {
    ...
    beforeEach(() => {
      ...
      overmind.actions.initializeRouter(guardRoutes)
    })

    test('allows access to public routes', () => {
      const result = overmind.actions.checkRouteAccess({
